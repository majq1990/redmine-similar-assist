...
# 排除的内部流程类 tracker（这类工单没实际技术内容）：
# 需求(2) / 代码审核(20) / UI设计(18) / 里程碑(15) / 日常(7) / 产品设计(17)
# 保留：支持/BUG/开发/更新/适配/安全/性能/低代码配置/内部系统开发/文档/运维 等
EXCLUDED_TRACKERS = {2, 20, 18, 15, 7, 17}
...
3. answer 字段是一段 markdown 文本（**200-400 字**），结构：
   - 先 1-2 段直接回答用户问题（基于候选内容，禁止编造；若候选不足以确定回答，说"现有资料不足以给出确定答案，建议补充xx关键词后重试"）
   - 然后列出 2-4 条关键要点（每条≤60 字）
   - 不要在 answer 里附链接（相关工单/文档链接由外层渲染）
...
def run_query(
    ...
    # 1. issues 召回：排除 6 类内部流程 tracker（需求/代码审核/UI/里程碑/日常/产品设计）
    vs = get_vector_store()
    issues_raw = vs.knn(emb, top=top_issues * 2, tracker_exclude=EXCLUDED_TRACKERS)
